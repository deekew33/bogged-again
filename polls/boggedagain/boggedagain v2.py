...
import os.path
import os, csv, pandas as pd, time, scrapy, bisect, logging, nltk, pickle, math
...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        # we start from page 0 on thestreet to get the full stock name
        urls = [f'https://www.thestreet.com/quote/{self.ticker}/details/news?page=0']
        # self.log(urls)
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        self.fullname = response.xpath('//h1[@class="quote__header__company-name"]/text()').get()
        for day in self.datesecs:
            self.datestring = time.strftime("%d/%m/%Y", time.localtime(day))
            namestringlist = self.fullname.replace('.', '').split() # replace periods with nothingness and split
            namestring = '+'.join(namestringlist) # rejoin the string with + signs for bing
            url = f'https://www.bing.com/news/search?q={namestring}+{datestring.replace("/","%2F")}&form=NWRFSH'
            print(url)
            yield scrapy.Request(url=url, callback=self.bingparse)

    def bingparse(self, response):
        try:
            smolname = self.fullname[0:self.fullname.index(' ')]  # pick first string in the full name as the string to check
        except:
            smolname = self.fullname  # if you can't find a space in the company name, chances are it's just one word
        rdatesecs = []
...
def extract_quote(ticker):
    ...
    if not isinstance(requested, str):
        ...
        for x in range(1, len(requested)):
            requested['Percent'][x] = math.ceil((requested['Close'][x] - requested['Close'][x - 1]) / requested['Close'][x - 1] * 10000)/100
    return requested
...
def newscrape(requested, ticker):
    ...
    for x in range(0, len(requested)-3):
        if requested['Percent'][x] <= targetdrop:
            final = math.ceil((requested["Close"][x+3]-requested["Close"][x])/requested["Close"][x]*10000)/100
            final_open = math.ceil((requested["Open"][x+3] - requested["Close"][x]) / requested["Close"][x] * 10000)/100
