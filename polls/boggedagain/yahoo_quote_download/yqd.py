...
def load_yahoo_quote(ticker, begindate, enddate, info = 'quote', format_output = 'list'):
	...
	def reqcreation(ticker, begindate, enddate):
	...
	while success == 0:
		try:
			req = reqcreation(ticker, begindate, enddate)
			f = urllib.request.urlopen(req)
			alines = f.read().decode('utf-8')
			success = 1
			...
			if text.find('404') != -1:
				success = 1
				return "Received 404 error. Are you sure this is the right ticker?"
			elif text.find('401') != -1:
				time.sleep(5)
				cookier = urllib.request.HTTPCookieProcessor()
				opener = urllib.request.build_opener(cookier)
				urllib.request.install_opener(opener)
